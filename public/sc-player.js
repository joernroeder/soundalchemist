...
(function($) {
  ...
  var apiKey,
      ...
      artworkImage = function(track, usePlaceholder) {
        if(usePlaceholder){
          ...
          return '<img src="' + track.artwork_url.replace('-large', '-t200x200') + '"/>';
        }else{
      ...
      updateTrackInfo = function($player, track) {
        ...
        $('.sc-artwork-list li', $player).each(function(index) {
          ...
          if (itemTrack === track) {
            ...
            $('.half, .full', $player).html($item.html());
  ...
  $.scPlayer = function(options, node) {
    var opts = $.extend({}, $.scPlayer.defaults, options),
        ...
        $half = $('<div class="half"></div>').appendTo($flip),
        $full = $('<div class="full"></div>');

        //$artworks = $().add($half).add($full);

        $flip.append($controls).append($full).prependTo($player);
